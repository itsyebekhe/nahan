...
async function d1Get(env, key) {
  ...
  await d1Init(env);
  try {
    const { results } = await env.IOT_DB.prepare("SELECT value FROM kv_store WHERE key = ?")
      .bind(key)
      .all();
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (configRegistry.size > 10000) {
        configRegistry.clear();
        trojanHashCache.clear();
      }
      ...
      if (!isTelemetryStream) {
        if (reqPath === routes.dash) {
          const dashboardUrl =
            env.DASHBOARD_URL ||
            "https://raw.githubusercontent.com/itsyebekhe/nahan/main/dashboard.html";
          try {
        ...
        if (reqPath === routes.data) {
          ...
          if (hasMultiUser) {
            if (targetSub) {
              targetUser = sysConfig.users.find(
                (u) => u.name.toLowerCase() === targetSub.toLowerCase() || u.id === targetSub,
              );
          ...
          if (isRealBrowser && !isCustomUaAllowed) {
            if (isValidUser) {
              const subscriptionUrl =
                ...
                "https://raw.githubusercontent.com/itsyebekhe/nahan/main/subscription.html";
              try {
                ...
                const totalPercent = limitTotal
                  ? Math.min(100, (totalReqs / limitTotal) * 100).toFixed(1)
                  : "0";
                const dailyPercent = limitDaily
                ...
                if (targetUser.userPanelUrl && targetUser.userPanelUrl.trim())
                  panelUrlToUse = targetUser.userPanelUrl.trim();
                if (panelUrlToUse) {
          ...
          if (isClashYaml) {
            ...
            return new Response(await buildYamlProfile(clientHost, targetSub, allowInsecure, env), {
              headers: resHeaders,
            });
          ...
          try {
            ...
            if (deployResult.success) {
              await logActivity(
                env,
                "Auto-Update Success",
                `Auto-updated to v${remoteVer} (encoded)`,
              );
...
async function sendTelegramMessage(request, type, hostName) {
  ...
  const inline_keyboard = [
    ...
    [
      ...
      {
        text: isPaused ? `▶️ ${locT("btn_resume")}` : `⏸️ ${locT("btn_pause")}`,
...
async function handleUsersApi(request, env, ctx) {
  try {
    ...
    if (method === "POST" && !userId) {
      ...
      ctx?.waitUntil(
        logActivity(env, "User Created", `User "${name}" (${newId}) created via API`).catch(
          () => {},
        ),
      );
      const hostName = new URL(request.url).hostname;
    ...
    if (method === "PUT" && userId) {
      const body = await request.json();
      if (!sysConfig.users)
        return new Response(JSON.stringify({ success: false, error: "No users" }), {
          status: 400,
          headers: { "Content-Type": "application/json" },
        });
      ...
      if (body.dailyLimit !== undefined)
        u.limitDailyReq = body.dailyLimit ? Math.floor(parseFloat(body.dailyLimit) * 6000) : null;
      if (body.expiryDays !== undefined)
        u.expiryMs = body.expiryDays ? Date.now() + parseInt(body.expiryDays) * 86400000 : null;
      if (body.notes !== undefined) u.notes = body.notes;
      if (body.maxConfigs !== undefined)
        u.maxConfigs = body.maxConfigs ? parseInt(body.maxConfigs) : null;
      ...
      await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
      ctx?.waitUntil(
        logActivity(env, "User Updated", `User "${u.name}" (${userId}) updated via API`).catch(
          () => {},
        ),
      );
    ...
    if (method === "DELETE" && userId) {
      ...
      return new Response(JSON.stringify({ success: true, deleted: deleted.id }), {
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (method === "POST" && userId && action === "reset") {
      ...
      ctx?.waitUntil(
        logActivity(env, "Traffic Reset", `Traffic reset for user ${userId} via API`).catch(
          () => {},
        ),
      );
      return new Response(JSON.stringify({ success: true, message: "Traffic reset" }), {
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (data.action === "deploy") {
      ...
      if (!newVersion) {
        ...
        if (versionMatch) {
          ...
          try {
            const vRes = await fetch(`https://raw.githubusercontent.com/${repo}/main/version`);
            if (vRes.ok) {
      ...
      if (deployResult.success) {
        ctx?.waitUntil(
          logActivity(env, "Panel Updated", `v${CURRENT_VERSION} → v${newVersion} (encoded)`).catch(
            () => {},
          ),
        );
                  ...
                  fetch(targetUrl, {
                    ...
                    .then(async (r) => {
                      ...
                      await logActivity(
                        env,
                        "Node Update Success",
                        `Node ${p.url} update response: ${JSON.stringify(resJson)}`,
                      );
                    })
                    .catch((e) => {
                      logActivity(
                        env,
                        "Node Update Failed",
                        `Node ${p.url} update failed: ${e.message}`,
                      );
                    }),
                );
              } catch (err) {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (body.action === "create") {
        ...
        await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
        ctx?.waitUntil(
          logActivity(env, "API Key Created", `Key "${newKey.name}" created`).catch(() => {}),
        );
      ...
      if (body.action === "revoke") {
        ...
        await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
        ctx?.waitUntil(
          logActivity(env, "API Key Revoked", `Key "${revoked.name}" revoked`).catch(() => {}),
        );
        return new Response(JSON.stringify({ success: true, revoked: revoked.id }), {
          headers: { "Content-Type": "application/json" },
        });
      }
    }
  ...
  try {
    ...
    if (isKeyAuth) {
      ...
      if (!sysConfig.silentAlerts && ctx)
        ctx.waitUntil(sendTelegramMessage(request, "ورود به پنل (موفق)", hostName));
      ...
      if (sysConfig.hubPanelUrl && sysConfig.hubPanelUrl.trim() && sysConfig.tgAdminId) {
        try {
          ...
          ctx?.waitUntil(
            fetch(`${hubUrl}/${encodeURI(sysConfig.apiRoute)}/tg/sync_panel`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
...
async function handleConfigSync(request, env, ctx) {
  try {
    ...
    if (!env.IOT_DB)
      return new Response(JSON.stringify({ success: false, msg: "DB Error" }), { status: 400 });
    ...
    if (data.config) {
      ...
      if (
        ...
        (!data.config.panelApiKeys || data.config.panelApiKeys.length === 0)
      ) {
    ...
    if (data.config && !data.fromMaster) {
      ...
      if (nextConfig.slaveNodes && nextConfig.slaveNodes.trim().length > 0) {
        ...
        nodes.forEach((node) => {
          if (node !== currentHost) {
            ctx?.waitUntil(
              fetch(`https://${node}/${encodeURI(nextConfig.apiRoute)}/api/sync`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  key: syncKey,
    ...
    if (nextConfig.tgToken && ctx) {
      ...
      ctx.waitUntil(
        fetch(`https://api.telegram.org/bot${nextConfig.tgToken}/setWebhook`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
async function handleSyncPanel(request, env, ctx) {
  try {
    ...
    if (!data.signal || data.signal !== "panel_login") {
      return new Response(JSON.stringify({ success: false, error: "Invalid signal" }), {
        status: 400,
      });
    }
...
async function handleTelegramWebhook(request, env, hostName, ctx) {
  try {
    ...
    const callerId =
      update.callback_query?.from?.id?.toString() || update.message?.from?.id?.toString();
    const adminId = sysConfig.tgAdminId || sysConfig.tgChatId;
    ...
    const getMainMenu = (activePanel, isAdmin = true) => {
      ...
      inline_keyboard.push([
        ...
        {
          text: isPaused ? `▶️ ${t("btn_resume")}` : `⏸️ ${t("btn_pause")}`,
          callback_data: "sys_toggle_status",
        },
      ]);
      if (panelUrl) {
    ...
    const getSubDetail = (uuid, usersList = null) => {
      ...
      const cleanIpsTxt = u.cleanIp
        ? u.cleanIp.substring(0, 30) + (u.cleanIp.length > 30 ? "..." : "")
        : "—";
      const proxyIpsTxt = u.proxyIp
        ? u.proxyIp.substring(0, 30) + (u.proxyIp.length > 30 ? "..." : "")
        : "—";
      const nodesTxt = u.userNodes
        ? u.userNodes.substring(0, 30) + (u.userNodes.length > 30 ? "..." : "")
        : "—";
      ...
      const kb = {
        inline_keyboard: [
          [
            {
              text: u.isPaused ? `▶️ ${t("btn_resume")}` : `⏸️ ${t("btn_pause")}`,
    ...
    if (update.callback_query) {
      ...
      if (chatId) {
        ...
        if (data === "main_menu") {
          ...
          sysConfig.apiRoute = Array.from(crypto.getRandomValues(new Uint8Array(8)))
          ...
          const cleanTxt = sysConfig.cleanIps
            ? sysConfig.cleanIps.substring(0, 40) + (sysConfig.cleanIps.length > 40 ? "..." : "")
            : "—";
          const lpUrls = (sysConfig.linkedPanels || []).map((p) => p.url).filter(Boolean);
          const nodesTxt =
            lpUrls.length > 0
              ? lpUrls.join(", ").substring(0, 40) + (lpUrls.join(", ").length > 40 ? "..." : "")
              : "—";
          ...
          const kb = {
            inline_keyboard: [
              ...
              [
                {
                  text: "type-user-port",
                  callback_data: "tg_set_strategy:type-user-port",
                },
              ],
              [
                {
      ...
      if (isAuthorized) {
        ...
        if (state) {
          ...
          if (state.step === "tg_edit_dns") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_dns")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_relay") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_relay")}: \`${text || "—"}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_nat64") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_nat64")}: \`${text || "—"}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_maintenance") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_maintenance")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_clean_ips") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_clean_ips")}: \`${text || "—"}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_prefix") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_prefix")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_pass") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_pass")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_strategy") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_strategy")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_tg_admin") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_tg_settings")} saved!`, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_cf_worker") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_cf_settings")} saved!`, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
          ...
          if (state.step === "tg_edit_ports") {
            ...
            tgState[chatId] = null;
            ctx?.waitUntil(d1Put(env, "tg_bot_state", JSON.stringify(tgState)).catch(() => {}));
            await sendOrEdit(chatId, `✅ ${t("tg_ports")}: \`${text}\``, {
              inline_keyboard: [
                [
                  {
                    text: "◀️ " + t("btn_back"),
        ...
        if (matchedUser) {
          ...
          const notFound =
            langCode === "fa" ? "کاربری با این شناسه یافت نشد." : "No user found with this ID.";
...
async function startDataPipe(webSocket, env, ctx, wsRelayIdx) {
  ...
  let clientConnectionRegistered = false;
  let isCleanedUp = false;

  /**
   * Cleanup تمام منابع مربوط به این اتصال.
   * این تابع idempotent است و فقط یک بار اجرا می‌شود.
   */
  async function cleanup() {
    ...
    // Decrease global connection count safely
    activeConnections = Math.max(0, activeConnections - 1);

    // Decrease per-client connection count only if registered
    if (clientConnectionRegistered && activeClientHash) {
      const current = activeConns.get(activeClientHash) || 0;

      if (current <= 1) {
        activeConns.delete(activeClientHash);
      } else {
        activeConns.set(activeClientHash, current - 1);
      }

      clientConnectionRegistered = false;
    }
  }

  /**
   * Safely close WebSocket and cleanup resources.
   */
  ...
  webSocket.addEventListener("close", () => {
    const cleanupPromise = cleanup().catch(() => {});
   ...
   * Message handler
   *
   * تمام پیام‌ها به صورت sequential پردازش می‌شوند
   * تا race condition بین initial packet و data packet
   * ایجاد نشود.
   */
      ...
      .then(async () => {
        ...
        try {
          if (isInit) {
            ...
            if (isModeAlpha && !isCleanedUp) {
              safeSend(new Uint8Array([0, 0]));
            }

            return;
          }

          if (!dataWriter || isCleanedUp) {
            return;
          }
  ...
  async function parseSensorData(bufferData, wsRelayIdx) {
    try {
      if (!bufferData || typeof bufferData.byteLength !== "number") {
        await closeConnection();
        return false;
      }
      ...
      if (view[0] === 0x00) {
        ...
        // Basic minimum length validation
        if (view.byteLength < 20) {
          await closeConnection();
          return false;
        }

        const clientHash = Array.from(view.slice(1, 17))
          .map((b) => b.toString(16).padStart(2, "0"))
          .join("");

        const configEntry = lookupConfigEntry(clientHash);

        if (configEntry) {
          activeClientHash = configEntry.userId.replace(/-/g, "").toLowerCase();

          activeProfile = getAllProfiles().find(
            (p) => p.id.replace(/-/g, "").toLowerCase() === activeClientHash,
          );
          ...
          if (decoded) {
            activeProfile = getAllProfiles().find((p) =>
              p.id.replace(/-/g, "").toLowerCase().startsWith(decoded.userFingerprint),
            );
        ...
        // Track usage without allowing tracking errors
        // to crash the Worker connection.
        try {
          const trackingPromise = trackUsage(activeClientHash, 0, env, ctx);
         ...
         * Connection limit
         */
        const currentConns = activeConns.get(activeClientHash) || 0;

        if (activeProfile.connLimit && currentConns >= activeProfile.connLimit) {
          await closeConnection();
          return isModeAlpha;
        }
        ...
        clientConnectionRegistered = true;

        /**
         * Usage tracking
         */
        const uTrack = uuidUsage.get(activeClientHash) || {
          connects: 0,
          last: 0,
        };
        ...
        if (pPos + 3 > bufferData.byteLength) {
          await closeConnection();
          return false;
        }
        ...
        if (aType === 1) {
          ...
          if (vPos + aLen > bufferData.byteLength) {
            await closeConnection();
            return false;
          }
        ...
        for (let i = 0; i + 1 < bufferData.byteLength; i++) {
        ...
        const configEntry = lookupConfigEntry(clientHashHex);
        ...
        if (configEntry) {
          ...
          if (wsRelayIdx >= 0) {
            ...
            if (effectivePips.length > 0) {
              activeProfile = {
                ...activeProfile,
                proxyIp: effectivePips[wsRelayIdx % effectivePips.length],
              };
            }
          }
        }
        ...
        if (aType === 1) {
          ...
          if (hPos + aLen > bufferData.byteLength) {
            await closeConnection();
            return false;
          }
        ...
        if (hPos + 2 > bufferData.byteLength) {
          await closeConnection();
          return false;
        }
       ...
       */
      const isDomain =
        /^([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/.test(targetAddr) || /^[a-zA-Z0-9-]+$/.test(targetAddr);
      ...
      if (isDomain && sysConfig.customDns) {
        try {
          ...
          const dnsRes = await fetch(dohUrl.toString(), {
            headers: {
          ...
          if (dnsRes.ok) {
            ...
            if (dnsJson?.Answer && Array.isArray(dnsJson.Answer)) {
              const answer = dnsJson.Answer.find(
                (item) => item?.data && typeof item.data === "string",
              );

              if (answer) {
                connectAddr = answer.data;
              }
            }
          }
        } catch (_) {
          // DNS failure should not crash the Worker.
      ...
      try {
        ...
        // Close partially created socket before failover
        if (remoteSocket) {
          try {
            await remoteSocket.close();
          } catch (_) {}
      ...
      if (!connected) {
        ...
        if (pips.length > 1) {
          ...
          const hashStr = activeProfile?.id || "";
        ...
        /**
         * Maximum 3 relay attempts
         */
        const maxAttempts = Math.min(pips.length, 3);

        for (let attempt = 0; attempt < maxAttempts; attempt++) {
          const currentIndex = (startIndex + attempt) % pips.length;

          const currentProxy = pips[currentIndex];

          if (!currentProxy) continue;

          let altIP = "";
          let altPort = targetPort;

          /**
           * Basic host:port parser
           *
           * For IPv4/domain: host:port
           */
          const lastColon = currentProxy.lastIndexOf(":");

          if (lastColon > -1 && currentProxy.indexOf(":") === lastColon) {
            altIP = currentProxy.slice(0, lastColon);

            const parsedPort = Number(currentProxy.slice(lastColon + 1));
          ...
          try {
            const candidateSocket = connect({
              hostname: altIP,
              port: altPort,
            });

            await candidateSocket.opened;

            remoteSocket = candidateSocket;
            connected = true;
       ...
       */
      if (offset < bufferData.byteLength && !isCleanedUp) {
        try {
       ...
       *
       * مهم‌ترین بخش Patch:
       * pipeTo قبلاً بدون catch رها شده بود.
       */
      const pipePromise = remoteSocket.readable
        ...
        .catch(async () => {
          // Expected on connection termination.
          // Do not rethrow.
        })
        .finally(async () => {
      ...
      /**
       * Prevent floating promise / unhandled rejection.
       */
      if (ctx && typeof ctx.waitUntil === "function") {
...
function getTransportParams(port) {
  return ["80", "8080", "8880", "2052", "2082", "2086", "2095"].includes(port.toString())
    ? "none"
    : "tls";
}
...
function getCleanIpsWithNames(hostName, userCleanIps = null) {
  ...
  if (entries.length === 0)
    entries = [
      {
        ip: hostName.endsWith(".pages.dev") ? sysConfig.metricNode : hostName,
        name: "",
      },
    ];
  return entries;
  ...
  if (sysConfig.users && sysConfig.users.length > 0) {
    ...
    sysConfig.users.forEach((u) => {
      ...
      if (
        ...
        if (sysUsageCache.users[u.id.replace(/-/g, "").toLowerCase()].reqs >= u.limitTotalReq)
          skip = true;
      }
      if (
        u.limitDailyReq &&
        ...
        if (usr.lastDay === new Date().toISOString().split("T")[0] && usr.dReqs >= u.limitDailyReq)
          skip = true;
      }
      if (!skip) {
        list.push({
...
function getGlobalNodeHosts() {
  ...
  if (Array.isArray(sysConfig.linkedPanels))
    hosts.push(...sysConfig.linkedPanels.map(linkedPanelHost).filter(Boolean));
            ...
            if (sysConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
              ...
              if (effectiveMode === "alpha" || effectiveMode === "both") {
                let dvName = getUniqueName(
                  getConfigName("alpha", p.name, port, hName, ip, null, dcIndex, ipName, true),
                );
              ...
              if (effectiveMode === "beta" || effectiveMode === "both") {
                let dtName = getUniqueName(
                  getConfigName("beta", p.name, port, hName, ip, null, dcIndex, ipName, true),
                );
            ...
            if (sysConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
              if (isVless) {
                let tagStr = getUniqueName(
                  getConfigName("alpha", p.name, port, hName, ip, null, configIndex, ipName, true),
                );
                dynamicTags.push(tagStr);
              ...
              if (isTrojan) {
                let tagStr = getUniqueName(
                  getConfigName("beta", p.name, port, hName, ip, null, configIndex, ipName, true),
                );
                dynamicTags.push(tagStr);
                proxyGeoInfo.set(tagStr, getGeoInfo(ip));
                let randomJunk = Array.from(
  ...
  profiles.forEach((p) => {
    ...
    hostNamesToUse.forEach((hName) => {
      p.ipLists.forEach((ipList) => {
        ...
        if (maxCfg > 0)
          ips = calcEffectiveIps(ips, maxCfg, effectiveMode, effectivePorts, pips.length);
        let ipNameMap = {};
              ...
              if (effectiveMode === "alpha" || effectiveMode === "both") {
                ...
                let ob = {
                  ...
                  streamSettings: {
                    ...
                    tlsSettings:
                      sec === "tls"
                        ? { serverName: hName, allowInsecure: allowInsecure }
                        : undefined,
